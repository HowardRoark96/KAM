...
export const PATTERN_MESSAGE_MAP = {
  [PATTERN.URL.toString()]: 'Invalid website URL',
  [PATTERN.EMAIL.toString()]: 'Invalid email',
};
export const VALIDATION_ERRORS: { [key: string]: { (arg: ValidationErrors): string } | { (arg: string): string } } = {
  ...
  pattern: (error: ValidationErrors) => PATTERN_MESSAGE_MAP[error.requiredPattern] || 'Поле заполнено неверно',
